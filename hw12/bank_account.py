...
Allows registering clients, opening deposit accounts,
calculating deposit interest, and closing deposit accounts.
"""
...
class Bank:
    ...
    def register_client(self, client_id, name):
        if client_id in self.clients:
            raise ValueError(
                f"Client with the id {client_id} is already registered"
            )
    ...
    def open_deposit_account(self, client_id, start_balance, years):
        if client_id not in self.clients:
            raise ValueError(
                f"There is no client with the id {client_id} in the system"
            )
        if client_id in self.deposit_accounts:
            raise ValueError(
                f"The deposit with the id {client_id} is already registered"
            )
    ...
    def calc_deposit_interest_rate(self, client_id):
        ...
        if client_id not in self.deposit_accounts:
            raise ValueError(
                f"There is no deposit for the client with the id {client_id}"
            )
    ...
    def close_deposit(self, client_id):
        if client_id not in self.deposit_accounts:
            raise ValueError(
                f"No deposit account exists for client {client_id}"
            )
        del self.deposit_accounts[client_id]
        print(
            f"The deposit for the client with the id {client_id} is closed"
        )
